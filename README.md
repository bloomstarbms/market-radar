...
- CONTRACT-CLIFF rows (v0.30.0) carry enforcement:'contract' only when post-cliff claim clusters replayed on >=2/3 of past index cliffs (n>=3); one such row exists (ORDER). Sourced rows below the derived pressure floor (0.061% of supply per tranche), farming/staking-only, or UNVERIFIABLE BY MECHANISM (continuous-claim streams, index dates the chain contradicts — v0.30.1) are tracked silently; the coverage line splits sourced into pending vs unverifiable. Every verified row carries a DERIVED falsifier strength (chance rate of its own window vs replay record — v0.30.2); boot refuses a verified row without one.
- CHANNEL SPLIT (v0.31.0): the public channel carries FACTS and CALLS only; heartbeat, digest and watch verdicts go to the operator DM, which also reports "public pushes 24h".
-->
